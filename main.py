...
from PySide2 import QtCore # pip3 install PySide2
...
## Function to encrypt to Base64 code
########################################################################
def base64_wrt(code):
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Code Encryptor")
        self.setWindowIcon(QIcon(resource_path("icon.ico")))
        self.ui.copy_btn.clicked.connect(self.copy_text)
        ...
        # PAGE 1
        self.ui.select_btn.clicked.connect(lambda: self.ui.stackedWidget.setCurrentWidget(self.ui.page1))
        
        # PAGE 2
        self.ui.org_btn.clicked.connect(lambda: self.ui.stackedWidget.setCurrentWidget(self.ui.page2))

        # PAGE 3
        self.ui.enc_btn.clicked.connect(lambda: self.ui.stackedWidget.setCurrentWidget(self.ui.page3))

        # PAGE 4
        self.ui.var_enc_btn.clicked.connect(lambda: self.ui.stackedWidget.setCurrentWidget(self.ui.page4))

        # PAGE 5
        self.ui.save_btn.clicked.connect(lambda: self.ui.stackedWidget.setCurrentWidget(self.ui.page5))

        # PAGE 6
        self.ui.about.clicked.connect(lambda: self.ui.stackedWidget.setCurrentWidget(self.ui.page6))

        # copy original code to clipboard
        self.ui.copy_btn.clicked.connect(lambda: self.copy_text(self.copy_text(self.original_code)))
        
        # copy encrypted emoji code to clipboard
        self.ui.copy_btn_2.clicked.connect(lambda: self.copy_text(self.copy_text(self.encrypted_code)))

        # copy encrypted var code to clipboard
        self.ui.copy_btn_3.clicked.connect(lambda: self.copy_text(self.copy_text(self.var_enc_code)))
        ...
        self.ui.browse.clicked.connect(self.browse_btn)

        ## ON clicking save button ##
        self.ui.save_emj.clicked.connect(self.save_func_emj)

        ## ON clicking save button ##
        self.ui.save_var.clicked.connect(self.save_func_var)
    ...
    ## Function to save encrypted emoji code
    ########################################################################
    def save_func_emj(self):
        filename = QFileDialog.getSaveFileName(self, 'Save File', 'C:\\', ".py")
        if ".py" in filename[0]:
            filename = filename[0]
        else:
            filename = filename[0] + ".py"
        self.ui.save_loc_emj.setText(filename)
        with open(filename, 'w', encoding='utf-8') as writefile:
            writefile.write(self.encrypted_code)
        self.ui.stackedWidget.setCurrentWidget(self.ui.page5)

    ## Function to save encrypted emoji code
    ########################################################################
    def save_func_var(self):
        filename = QFileDialog.getSaveFileName(self, 'Save File', 'C:\\', ".py")
        if ".py" in filename[0]:
            filename = filename[0]
        else:
            filename = filename[0] + ".py"
        self.ui.save_var_2.setText(filename)
        with open(filename, 'w', encoding='utf-8') as writefile:
            writefile.write(self.var_enc_code)
        self.ui.stackedWidget.setCurrentWidget(self.ui.page5)

    ## Function run on click browse
    ########################################################################
    def browse_btn(self):
        filename = QFileDialog.getOpenFileName(self, 'Open File', ".py")[0]
        self.ui.org_loc.setText(filename)
        with open(filename, 'r', encoding='utf-8') as readfile:
            self.original_code = readfile.read()
        self.ui.stackedWidget.setCurrentWidget(self.ui.page2)
        self.ui.code_here.setText(self.original_code)
        self.encrypted_code = emoji_write(self.original_code, emoji)
        self.var_enc_code =  base64_wrt(self.original_code)
        self.ui.emj_code_here.setText(self.encrypted_code)
        self.ui.var_code_here.setText(self.var_enc_code)
        self.ui.save_loc_emj.setText("")
        self.ui.save_var_2.setText("")
        self.ui.toggle_btn.click()
